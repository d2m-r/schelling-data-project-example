...
Of the `r total_where_responses` total responses to the three "meetup location" questions, `r error_responses` were excluded and <span color="red">NUMBER GOES HERE</span> were re-coded as "Other".
...
**Task 1:** If you both choose the _SAME_ answer, you both win. Which do you choose?

**Task 2:** If you both choose _DIFFERENT_ answers, you both win. Which do you choose?
...
# 1) make the percent table
coin_flip_tab <-
  table(coins$side, coins$goal) |>
  prop.table(margin = 2) |>
  addmargins(margin = 1) |>
  round(3) * 100

## STOP HERE FOR A SIMPLE, PRINTABLE PROPORTIONS TABLE
## OR CONTINUE TO FORMAT AS A FLEXTABLE FOR PRETTIER MANUSCRIPT TABLES

# 2) convert to a data frame for flextable
coin_flip_df <- as.data.frame.matrix(coin_flip_tab)
coin_flip_df <- rownames_to_column(coin_flip_df, "Side")

# 3) print
coin_flip_df |>
  flextable() |>
  colformat_num(j = -1, digits = 1, suffix = "%", big.mark = "")
...
In selecting from an incremental set of integers, participants most frequently chose the number 7 (...), which was also the most frequently chosen number in Schelling's original survey. This number is often cited as a classic example of a Schelling point, as it is the most salient option in this set of choices.


**Task 2:** Pick a number from the list^[Participants were presented with the following list of numbers in a randomized order: 
`r levels(numbers$number_list)`.] below.
